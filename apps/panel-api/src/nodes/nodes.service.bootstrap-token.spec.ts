...
describe("NodesService bootstrap token lifecycle", () => {
  ...
  it("regenerate mints a fresh token, sets a future expiry, and clears used", async () => {
    const { bootstrapToken, expiresAt } =
      await svc.regenerateBootstrap("node-1");
  ...
  it("a regenerated token is then usable (used cleared)", async () => {
    ...
    const result = await svc.registerAgentByToken({
      bootstrapToken: "NEW-TOKEN",
    });
    expect(result.nodeId).toBe("node-1");
  });

  // The legacy /nodes/:id/register + /nodes/:id/heartbeat surface was removed:
  // the agent registers only via registerAgentByToken (above) and sends
  // heartbeats through the HMAC-signed /agent/heartbeat route. Guard against
  // it silently coming back.
  it("exposes no legacy registerAgent/ingestHeartbeat methods", () => {
    expect((svc as any).registerAgent).toBeUndefined();
    expect((svc as any).ingestHeartbeat).toBeUndefined();
  });
});
