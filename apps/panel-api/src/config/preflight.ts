...
export function evaluatePreflight(
  ...
  if (DEV_SECRET_DEFAULTS.has(accessSecret) || looksPlaceholder(accessSecret)) {
    ...
    errors.push('JWT_ACCESS_SECRET is too short (use ≥32 chars; openssl rand -hex 48).');
  }
  if (
    ...
    errors.push('JWT_REFRESH_SECRET is too short (use ≥32 chars; openssl rand -hex 48).');
  }
