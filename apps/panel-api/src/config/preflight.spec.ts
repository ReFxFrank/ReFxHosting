...
describe('production preflight', () => {
  ...
  const goodConfig = (): AppConfig =>
    ({
      ...
      paypal: { clientId: 'live-client-id', clientSecret: 'secret', mode: 'live' },
      billing: { invoiceNumberPrefix: 'INV', defaultCurrency: 'USD', schedulerEnabled: true },
      throttle: { ttl: 60, limit: 120 },
      apns: { keyP8: '', keyId: '', teamId: '', bundleId: '', production: true },
  ...
  it('flags a placeholder DB password and an empty DATABASE_URL', () => {
    ...
    expect(evaluatePreflight(c, true).errors.some((e) => e.includes('DATABASE_URL'))).toBe(true);
    c.database.url = '';
    expect(evaluatePreflight(c, true).errors.some((e) => e.includes('DATABASE_URL'))).toBe(true);
  });
