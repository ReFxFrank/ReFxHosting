      ...
      <Reveal>
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-3 max-w-xl text-sm text-muted-foreground">
            The people behind ReFx Hosting — building the platform and keeping your
            servers online around the clock.
          </p>
          {team.length > 0 && (
            <div className="mt-6">
              <AvatarGroup
                items={team.map((m) => ({ name: m.name, avatarUrl: m.avatarUrl }))}
                size={48}
              />
            </div>
          )}
...
function TeamCard({ member }: { member: TeamMember }) {
  return (
    <div className="refx-beam group relative flex h-full flex-col items-center gap-3 rounded-2xl border border-white/[0.07] bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.015))] p-6 text-center transition-transform duration-300 ease-out hover:-translate-y-1">
