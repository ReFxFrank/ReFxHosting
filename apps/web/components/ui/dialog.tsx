  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        //
        // max-h + overflow-y-auto: long forms (e.g. the node edit dialog) are
        // taller than a phone screen. Without a height cap the centered dialog
        // pushes its title/footer off both edges of the viewport and — with the
        // old `overflow-hidden` — there was no way to scroll to them. Cap at
        // 90dvh (dynamic vh so mobile browser chrome doesn't clip it) and let
        // the content scroll internally.
        "refx-beam fixed left-1/2 top-1/2 z-50 grid max-h-[90dvh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto overflow-x-hidden border border-white/[0.08] bg-[linear-gradient(180deg,rgba(15,24,40,0.98),rgba(7,13,24,0.98))] p-6 shadow-[0_1px_0_rgba(255,255,255,0.06)_inset,0_40px_90px_-30px_rgba(0,0,0,0.9)] duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 sm:rounded-xl",
        className,
      )}
      {...props}
    >
