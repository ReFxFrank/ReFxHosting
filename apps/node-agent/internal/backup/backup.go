...
func (m *Manager) Create(ctx context.Context, backupID, dataDir string, ignoredGlobs []string, progress ProgressFunc, kind string) (*Result, error) {
	...
	walkErr := filepath.Walk(dataDir, func(path string, info os.FileInfo, err error) error {
		...
		// Symlinks: record the link itself; NEVER follow it. A Proton/wine prefix's
		// dosdevices symlinks (e.g. `z: -> /`) would otherwise make os.Open+io.Copy
		// below read the entire host filesystem and fail the whole backup.
		if info.Mode()&os.ModeSymlink != 0 {
			link, lerr := os.Readlink(path)
			if lerr != nil {
				return nil // unreadable link — skip rather than fail the backup
			}
		...
		// Skip irregular files (sockets, FIFOs, char/block devices): tar can't
		// represent them and they're meaningless to restore (e.g. a wineserver
		// socket). Only regular files and directories proceed.
...
func (m *Manager) Restore(ctx context.Context, location, dataDir string, progress ProgressFunc) error {
	...
	for {
		...
		// Do NOT recreate symlinks/hardlinks on restore: a later archive entry
		// could otherwise be written THROUGH a restored symlink and escape the
		// jail. Wine/Proton prefixes (the only place these appear) are excluded
		// from backups and regenerated by the game, so skipping is safe.
