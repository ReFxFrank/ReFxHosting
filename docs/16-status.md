...
| Steam Workshop | **Done (panel)** | Per-server **Workshop tab** for Steam games (`supportsWorkshop` templates: Garry's Mod, Arma 3, Project Zomboid, DayZ, CS2): add items/collections by ID or URL, enable/reorder/remove, and **Apply** (writes `WORKSHOP_*` server vars + reinstalls so the egg's startup/steamcmd consumes them). **Central Steam login + Web API key** (Admin → Settings → Steam, encrypted) injected into the install env for Workshop downloads. Collection/name resolution via Steam's public Web API (best-effort). Per-game steamcmd download + mod-load wiring lives in the eggs (GMod runtime collection works as-is; Arma 3 egg downloads items — needs a central login that owns the game). |
