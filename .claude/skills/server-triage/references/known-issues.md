...
- **Searchable error string**: `no .rpt written — crash was before Arma init` … `A code 139 here = missing/incompatible steamclient.so`; the fixed launcher's verdict for the underlying failure reads `arma3server exited (code 137)` + `OUT OF MEMORY: the kernel's OOM killer fired`
...
- **Confirmed instance (2026-08-04)**: Antistasi + ~42 mods + 3 headless clients on a ~16 GB plan. First run with the fixed launcher printed `exited (code 137) after 75s` and `OOM killer fired 1 time(s)` — the OOM hit during the HCs' mod-loading ramp (~35s after they connect). Resolution: fewer HCs or a larger plan; each HC costs roughly a full game instance of RAM. The HC purchase card now states this cost up front.
