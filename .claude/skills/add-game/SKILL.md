...
## Project facts (fill these in once, then delete the TODOs)

These are the only things this skill cannot know. Fill them in and every future game inherits them.

- TODO(frank): Panel/orchestrator — Pterodactyl / Pelican / custom? Where do game definitions live (repo path, file format)?
- TODO(frank): Container image registry + base image convention (e.g. `ghcr.io/refx/yolks:java_21`).
- TODO(frank): Repo path for the games catalog / metadata (name, slug, icon, category, tags).
- TODO(frank): Repo path for customer docs, and the docs framework.
- TODO(frank): Repo path for marketing site + landing pages.
- TODO(frank): Where plan/pricing tiers are defined.
- TODO(frank): Port allocation strategy (fixed ranges per node? dynamic allocation from a pool?).
- TODO(frank): Backup system — what it snapshots, and how include/exclude paths are declared.
- TODO(frank): Staging environment for provisioning a test server (or "prod with an internal account").
